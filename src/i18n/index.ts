...
// 语言设置 → i18n 实例（settings.language 写入后两窗口即时切换）
watchEffect(() => {
  const lang = sget("language") as AppLocale;
...
export function randomQuote(): string {
  const quotes = i18n.global.tm("pet.quotes") as unknown[];
