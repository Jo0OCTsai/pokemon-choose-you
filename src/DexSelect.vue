...
<style scoped>
.dex-select { position: relative; flex: none; }
.ds-btn {
  display: flex; align-items: center; gap: 8px;
  border: 3px solid var(--dex-navy); border-radius: 8px;
  background: #fff; color: var(--dex-navy);
  font-size: 13px; font-weight: 700; font-family: inherit;
  padding: 7px 10px; min-height: 38px; min-width: 150px;
  cursor: pointer; box-shadow: 3px 3px 0 var(--dex-navy);
}
.ds-btn:active { transform: translate(1px, 1px); box-shadow: 2px 2px 0 var(--dex-navy); }
.ds-btn.open { background: var(--poke-yellow); }
...
.ds-list li {
  ...
  font-size: 13px; font-weight: 700; color: var(--dex-navy);
  padding: 8px 8px; border-radius: 6px; cursor: pointer; white-space: nowrap;
}
.ds-list li:hover { background: #fff3c4; }
.ds-list li.sel { background: var(--poke-yellow); }
.ds-cursor { width: 10px; flex: none; font-size: 9px; opacity: 0; }
.ds-list li.sel .ds-cursor { opacity: 1; }
</style>
