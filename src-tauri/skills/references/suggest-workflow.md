...
- 人名已代号化：prompt 与 `pk context` 里的真名都替换成了稳定代号（如 `成员_a1b2`），同代号=同人、「我」=用户本人；title/note/reason 沿用代号回写（落库自动还原真名），不要猜测真实姓名
- 消息头的「提及：@我（显式）/ 疑似@我」是归属判定的依据：显式提及默认视为指派给用户（除非内容明确转给别的代号）；疑似提及结合上下文判断；无提及标注的群聊消息判 none
- 所有 id 必须来自 prompt 的消息列表或 `pk context` 的 openTasks，不要猜测或编造
- 分类必须存在于 context；title 用不超过 20 字的祈使句中文
- tags 按维度选 0~3 个：词表内标签用对象 `{"name","dimension","isNew":false}`；某维度没有贴切选项且消息有明确依据（出现的项目名/人名/群名）时可提议新标签（`isNew:true`），模糊语境复用现有标签或整个省略 tags；`pk context` 的 dimensions 里 remaining<=0 的维度禁止新建
- 需要变更标签时给出完整的新数组；不适用的字段整个省略——任何字段都不要填空字符串 `""`（会导致整批解析失败），`updateTaskId`/`followUpTaskId` 只能是数字或省略
