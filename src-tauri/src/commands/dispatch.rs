...
// ---- 远端派发预检：一条 ssh 拿 tmux / 目录 / claude 信任状态 ----
...
fn tmux_probe_line(workdir: &str) -> String {
...
fn parse_remote_probe(out: &str) -> RemoteProbe {
...
/// 判定不提示（宁可漏报不误报）
fn local_trust_note(kind: Option<&str>, dir: Option<&std::path::Path>) -> Option<String> {
    let dir = dir?;
...
mod tests {
    ...
    // ---- 交互通道的信任预检（claude-code 首启停在信任框是「派发了却无响应」的头号来源） ----
