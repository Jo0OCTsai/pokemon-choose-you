...
- 0.x 阶段数据格式（SQLite schema）不承诺稳定，升级前请备份 `pokemon-knock.db`（P2 将提供应用内导出）。
...
- **本地数据**：任务/设置明文存储于本机 SQLite；AI / 飞书 / Todoist 凭证同样本地存储，仅用于直连对应服务，不上传到任何第三方。使用自建 AI 接口时请注意数据会流经你所配置的服务商。
