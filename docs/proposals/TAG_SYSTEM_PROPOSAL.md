...
>
> **实施状态**：P1–P5 已全部落地。
> - P1–P3：维度模型与种子、AI 维度化打标与 isNew 提议、接受即建标签、
>   收音机/编辑弹窗/设置页/任务卡 UI、pk tag create、nlCapture #新名字新建、i18n、技能文档。
> - P4 标签体检：复盘向导「标签体检」步骤（`tag_health.rs`）——同维度近义合并候选
>   （编辑距离相似度 ≥0.7 预筛、上限 20 对；配置 agent 时 LLM 复核并给理由），用户逐条
>   确认合并；僵尸标签（90 天未挂任务且 AI/捕捉/agent 所建）建议**放生**（删除，未引入
>   归档态，与原文「归档」的差异）；采纳新维度建议即建维度并迁移既有标签。
> - P5 反馈 few-shot：task_logs 聚合近 30 天被移除 ≥2 次的标签（封顶 3 个）注入 pk
>   context 的 tagFeedback，分类 prompt 据此提示「无新的明确依据不要再建议」。
>   保存视图、project 平移独立 projects 表为可选项，未实施。
...
反馈 few-shot 已随 P5 落地：用户移除 AI 标签的记录聚合（30 天窗口、≥2 次、封顶 3）注入
分类上下文作负反馈（「无明确依据不要再建议」），阈值与封顶即防错误反馈固化的手段。
