...
- **免打扰过滤**（`chat_mute_outcomes`）：折叠状态开放平台未暴露，用免打扰做代理信号（折叠的噪音会话通常也设了免打扰），整会话不拉取；查询失败降级为不过滤，不阻断链路。（✅ 2026-09 起免打扰降为**默认值**：设置 → 集成 → 「会话过滤」可逐会话三态覆盖（跟随/总是拉取/总是过滤），手动设置后以本应用为准——见 `specs/feishu-chat-filter/`）
