...
1. **建一个任务**：图鉴机顶部输入「要捕捉哪只野生宝可梦？」，选分类、优先级、时间，开关拨到「丢进草丛」或「加入路线」，点 **＋ 捕捉**
   - **偷懒写法（自然语言）**：直接输入「明天 5pm 交周报 #工作」——时间、分类、标签自动识别，输入框下方出现**高亮预览**（识别成什么一目了然），看着不对点 **✕ 取消识别** 按原文保存；设置 → 通用里可整体关闭。支持今天/明天/后天、（下）周X、X月X日、5pm、下午3点半等写法，`#名字` 认已有的分类与标签直接命中；词表外的 `#新名字` 预览标「＋ 将新建」，保存时按主题维度建标签。
