...
每个任务分类绑定一只宝可梦（默认：工作=皮卡丘、学习=可达鸭、生活=妙蛙种子、健康=吉利蛋、社交=伊布、紧急=卡比兽），桌宠显示的精灵随当前任务的分类自动切换。分类可在设置里换装成 **PokeAPI 全部 1025 只**中的任意一只（内置 6 只离线可用，其余首次展示时联网加载精灵图）；没有任务时桌宠展示**主宝可梦**（设置 → 分类，不设置则跟随第一个分类），每只宝可梦还能写**专属台词**（点击桌宠时随机冒出）。
