...
| **双击精灵** | 打开图鉴机主面板 |
| **右键精灵** | 菜单：打开图鉴机 / 快捷图鉴屏 /（配置了 AI 时）💬 和精灵聊聊 / 暂停当前任务 / 隐藏桌宠 |
| 番茄钟 ⏸ / ▶ | 暂停 / 恢复当前任务计时 |
| 番茄钟 ✔ | 完成当前任务 |
| 番茄钟 ⇄ | 弹出切换目标列表 |
...
新建待办的唯一入口在**收音机**页顶部：一句自然语言（「明天下午 5 点交周报」「周五前给老妈订生日蛋糕 #生活」）→ **⚡ 发报**。AI agent 判定标题、分类、标签、截止时间、优先级与备注：新待办直接建好（识别出截止时间进路线，否则进草丛；5 秒内 ↩ 可撤销）；命中现有待办则不重复建，生成更新建议卡或并入跟进，在收音机里等确认。判定失败的消息保留为「AI 判定失败」，可强制捕捉或逃走。`Ctrl/Cmd+Shift+K` 全局直达。
...
| **🏷 标签** | 标签按**维度**分组管理（内置项目/场景/人物/主题，可增改）：项目维度单选、每条待办至多 3 个标签、每维度有数量上限。AI 从消息生成待办时会挑选合适的标签；某维度没有贴切选项且消息有明确依据（出现的项目名/人名/群名）时会提议新标签——建议卡上标「＋」，**接受建议时才创建**，不接受不进词表 |
| **🗂 标签维度** | 维度改名/上限/停用；key 与单多选建后不可改（AI 协议稳定标识）。停用后该维度不进新建与 AI 选项 |
