...
- **收音机**——从飞书电波里挑出新的待办
- **日志**——agent 会话的战斗实录（分类 / 捕捉 / 派发），按来源过滤、随时回放
...
- **悬停精灵唤出快捷图鉴屏**：LCD 屏今日列表，点选即「出发」，全程不用打开主窗口
- 搭话与撸宠：单击精灵蹦跳搭话、长按 600ms 撸宠（每只宝可梦可写专属台词），台词气泡按时长自动淡出不打扰；任务相关的气泡与提醒会用当前宝可梦的名字说话，开始任务喊「就决定是你了！」

### 图鉴机（主面板）
- 图鉴条目卡：条目编号、属性徽章（分类）、优先级圆点（紧急闪烁）、专注时长累计、今日捕捉进度条
- 分级提醒：普通 = 气泡 + 系统通知；紧急 = 桌宠抖动敲门。提醒调度在 Rust 后台，关掉面板不漏提醒
- 收音机：飞书消息 → AI 识别新待办/现有待办变更/跟进记录（判定链路全链路假名化，真名与群名不进大模型）→ 逐条「◎ 捕捉 · 应用更新 / ✕ 逃走」，支持按待处理批量分诊，判定失败可一键批量重判，不强迫决策
...
- 设置中心：专注 / 分类与标签（宝可梦换装 · 标签 · 项目派发）/ 桌宠（主宝可梦 · 台词 · 输入响应）/ 显示 / Agent / 飞书 / 诊断 / 通用 八个分区
...
### 本地优先与可靠性
- 数据存在本机（SQLite），无云端副本、无账号体系
- **无遥测、无工具自有云依赖**：不采集使用数据、没有厂商自建后端——飞书 / AI 等外部能力全走你自己的配置与凭证；断网只影响这些外部链路，任务管理本身照常
- 判定链路全链路假名化：送大模型前真实姓名与群聊名替换为稳定代号（成员_xxxx / 群_xxxx），真名只存在本机数据库；桌宠问答与待办派发的 prompt 同规则脱敏
